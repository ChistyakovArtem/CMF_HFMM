...
class Strategy:
    """
        Future.py but we use Stoikov's shift in order to control adverse selection and inventory risks
        (classic Adelaida-Stoikov but future price instead of midprice)
    ...
    def __init__(self, delay: float, risk_koef, time_oi, avg_sum_oi, avg_time_oi, avg_volatility, min_asset_value,
        ...
        :param risk_koef:                       From Stoikov article (gamma)
        ...
        self.logs = {
            ...
            'future_price': [],  # future price after future_timestamp nanoseconds
        ...
        while True:
            ...
            if receive_ts - prev_time >= self.delay:
                ...
                if self.volatility is not None and self.scaled_order_intensity is not None:
                    ...
                    # (T - t) = 1
                    future_price = self.get_future_price(receive_ts)
                    indifference_price = future_price -\
                        (self.asset_position / self.normalizer) * self.risk_koef * self.volatility

                    my_spread = self.risk_koef * self.volatility + 2 / self.risk_koef *\
                        np.log(1 + self.risk_koef / self.scaled_order_intensity)

                    ask_place = indifference_price + my_spread / 2
                    bid_place = indifference_price - my_spread / 2

                    self.logs['future_price'].append(future_price)
