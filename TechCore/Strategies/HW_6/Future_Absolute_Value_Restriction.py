...
class Strategy:
    """
        Future.py, but in order to evade risks we restrict our max position
        (after we reach it - we stop placing orders in blocked position)
    ...
    def __init__(self, delay: float, risk_koef, time_oi, avg_sum_oi, avg_time_oi, avg_volatility, min_asset_value,
                 ...
                 abs_value_restriction, order_fees=-0.00001) -> None:
        ...
        :param abs_value_restriction            Abs asset position restriction (blocks trades in this direction)
        """
        ...
        self.abs_value_restriction = abs_value_restriction
        ...
        while True:
            ...
            if receive_ts - prev_time >= self.delay:
                ...
                if self.volatility is not None and self.scaled_order_intensity is not None:
                    ...
                    if self.asset_position > self.abs_value_restriction:
                        # we can't place bid order
                        bid_place = 0

                    if self.asset_position < -self.abs_value_restriction:
                        # we can't place ask order
                        ask_place = 10000000
